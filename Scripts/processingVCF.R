...
lista_genes <- list('CYP3A4', 'GSTM1', 'CYP17A1', 'UGT2B17', 'MTHFR', 'CYP1A1', 
                    'NAT1', 'NAT2', 'GSTT1', 'OGG1', 'TXNRD1', 'PRDX3', 'GSTP1', 
                    'GPX1', 'SOD2', 'GSTA1', 'GSTA2', 'GSTA3', 'GSTA4', 'GSTA5', 
                    'GSTT2', 'GSTM5', 'GSTM2', 'GSTM3', 'GSTM4', 'GSTK1', 'GSTO2', 
                    'GSTO1', 'SOD1', 'SOD3')
...
for (carpeta in manifiesto3$id) {
  wd <- paste0(wdini, '/Descargar/', carpeta) # Reads the name of each directory in the manifest file
  setwd(wd) 
...
# Next, an optional loop to check which individuals we have, based on their bcr identifier
salida <- data.frame()

for (carpeta in manifiesto3$id) {
  wd <- paste0(wdini, '/Descargar/', carpeta)
  setwd(wd)
  archiv <- dir(pattern = '.csv$') # Input file
  salida <- base::rbind(salida, unlist(strsplit(archiv, '_'))[1])
}
setwd(wdini)
write.csv(salida, "individuos.csv")
